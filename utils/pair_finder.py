...
class PairsFinder:
    ...
    def find_pairs(self, data: pd.DataFrame) -> List[Dict]:
        ...
        for i, (symbol1, symbol2) in enumerate(all_combinations):
            if i % 100 == 0:
                logger.info(f"Progress: {i}/{total_combinations} pairs tested.")
            
            pair_stats = self.analyze_pair(data, returns, symbol1, symbol2)
            if pair_stats and self.is_valid_pair(pair_stats):
                valid_pairs.append(pair_stats)
        
        valid_pairs.sort(key=lambda x: x['quality_score'], reverse=True)
        return valid_pairs

    def analyze_pair(self, data: pd.DataFrame, returns: pd.DataFrame, symbol1: str, symbol2: str) -> Optional[Dict]:
        ...
        _, pvalue, _ = coint(log_price1, log_price2)
        is_cointegrated = pvalue < self.config.cointegration_threshold
        ...
        if not (self.config.min_spread_std < spread_std < self.config.max_spread_std):
            return None

        half_life = self.calculate_half_life(raw_spread)
        sharpe_ratio = self.calculate_spread_sharpe(raw_spread)
        volatility_ratio = ret1.std() / ret2.std() if ret2.std() > 0 else 0
        quality_score = self.calculate_quality_score(abs(correlation), pvalue, spread_std, half_life, sharpe_ratio)

        return {
            'symbol1': symbol1, 'symbol2': symbol2, 'pair_name': f"{symbol1}-{symbol2}",
            'correlation': correlation, 'cointegration_pvalue': pvalue, 'hedge_ratio': hedge_ratio,
            'spread_mean': float(raw_spread.mean()), 'spread_std': spread_std, 'half_life': half_life,
            'sharpe_ratio': sharpe_ratio, 'volatility_ratio': volatility_ratio,
            'quality_score': quality_score, 'observations': len(common_index)
        }

    def is_valid_pair(self, pair_stats: Dict) -> bool:
        checks = [
            self.config.min_correlation <= abs(pair_stats['correlation']) <= self.config.max_correlation,
            pair_stats['cointegration_pvalue'] < self.config.cointegration_threshold,
            self.config.min_spread_std < pair_stats['spread_std'] < self.config.max_spread_std,
            pair_stats['observations'] >= self.config.min_observations,
            0 < pair_stats['half_life'] < 100,
        ]
    ...
    def calculate_quality_score(self, correlation: float, pvalue: float, spread_std: float, half_life: float, sharpe_ratio: float) -> float:
        corr_score = min(correlation, 1.0)
        coint_score = max(0, 1 - (pvalue / 0.05))
        ...
        sharpe_score = max(0, min(1, sharpe_ratio / 2.0))
